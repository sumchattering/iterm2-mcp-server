...
function parseTailLines(value: unknown): number {
  if (value === undefined) {
    return 10;
  }
    ...
    return {
      tools: [
        ...
        },
        {
          ...
          description:
            "Read the bottom lines from a specific iTerm2 pane. Trailing empty lines are ignored first, then up to tail_lines are returned from the visible screen buffer (not scrollback).",
          inputSchema: {
            type: "object",
            properties: {
              session_id: {
              ...
              tail_lines: {
                type: "integer",
                description:
                  "Number of lines to return from the bottom, anchored to the last non-empty visible line.",
                minimum: 1,
                maximum: 200,
                default: 10,
              },
    ...
    try {
      switch (name) {
        ...
        case "iterm2_glimpse_pane": {
          const { session_id: sessionId, tail_lines: tailLinesRaw } = args as {
            session_id?: string;
            tail_lines?: unknown;
          };
          ...
          const totalLines =
            typeof result.total_lines === "number" ? result.total_lines : null;
          const returnedLines =
            typeof result.returned_lines === "number"
